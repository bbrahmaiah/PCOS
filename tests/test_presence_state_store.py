...
def test_store_starts_with_default_state() -> None:
    ...
    assert snapshot.state == PresenceState()
...
def test_store_applies_valid_transition() -> None:
    ...
    assert store.current_state().mode == PresenceMode.LISTENING
    assert store.current_state().turn_phase == TurnPhase.LISTENING_FOR_USER
...
def test_store_records_rejected_transition_without_mutating_state() -> None:
    ...
    assert store.current_state() == PresenceState()
    assert snapshot.transition_count == 1
...
def test_store_sleep_reset_error_recover_flow() -> None:
    ...
    assert store.current_state().mode == PresenceMode.IDLE
    assert store.current_state().last_error is None
...
def test_store_initial_state_can_be_injected() -> None:
    ...
    assert store.current_state() == initial_state